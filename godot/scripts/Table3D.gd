...
@onready var background_sprite = $BackgroundPlane
...
func _ready():
	...
	if main_scene:
		...
		if main_scene.has_signal("battle_chapter_applied"):
			main_scene.battle_chapter_applied.connect(func(_chapter): _apply_backdrop_from_main(main_scene))
		_apply_backdrop_from_main(main_scene)
...
func _apply_backdrop_from_main(main_scene):
	var path = ""
	if main_scene.has_method("get_battle_background_path"):
		path = main_scene.get_battle_background_path()
	_apply_backdrop_texture(path)

func _apply_backdrop_texture(path: String):
